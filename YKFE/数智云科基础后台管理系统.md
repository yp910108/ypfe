...
> [数智云科基础后台管理系统](http://git.qiluys.cn/YKFE/yk-manage)基于开源项目[vue-element-admin](https://panjiachen.github.io/vue-element-admin-site/)
...
### 按钮权限
1.封装指令，根据当前路由递归查询找出对应的按钮，如果当前路由中没有给定点的按钮权限，则将此按钮删除。使用方法：v-has='add'
2.混入（mixins），根据当前路由递归查询找出对应的按钮，暴露出一个has方法，调用的时候传入（'add','edit',...）等参数（次参数是约定好的），判断当前传入的参数是否包含在当前的路由中。
        使用方法：在按钮标签中  v-if="has('add', 'edit')"
...
```js
<yk-pagination :total="total" :pageNo.sync="pageNo" :pageSize.sync="pageSize" @pagination="fetchList" />
```
...
- 考虑到上传图片和上传文件用到的最多，所以基于`yk-upload`封装了`upload-picture`和`upload-file`公用组件
...
```js
// 使用方式
<yk-icon icon="home" /> // icon 为 src/base/icon/svg 下图标文件的名字
```
...
<aside>
  <h3>下拉框</h3>
  审核状态：<yk-select v-model="status" :options="userAuditStatusOptions" />
</aside>
<aside>
  <h3>显示文字，并设置状态颜色</h3>
  审核状态：
