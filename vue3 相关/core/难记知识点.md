...
- 对于 `Boolean` 类型的 prop
  - 如果 attribute 的值不指定或指定为空字符串（`disabled=""`），那么 prop 为 `true`
  - 如果传入 `disabled` 或者不指定，无论声明类型的顺序如何，值都会按照 `Boolean` 类型的特殊转换规则进行转换
  - 如果传入 `disabled=""`，当 prop 的 `type` 设置为 `[Boolean, String]` 时，值为 `true`，当 prop 的 `type` 设置为 `[String, Boolean]` 时，值为空字符串
...
当 `prop` 为对象（字面量不可以），并且父组件传递的其他 `prop`（即使子组件没有声明该 `prop`）发生修改时，子组件通过 `watch` 监听该 `prop` 无需增加 `immediate: true` 也可以触发立即改变：

```html
<!-- App.vue -->
<template>
  <!-- foo 为字面量时不可以触发子组件的 watch -->
  <Test :foo="{ msg: 'this is foo.' }" :bar="bar" />
</template>
...
const bar = ref("this is bar.");

setTimeout(() => {
  bar.value = "this is baz.";
}, 1000);
</script>
```
...
在 `input` 中随便输入，按钮点击一秒后 `div` 会再次出现，此时 `input` 里面的内容并没有清空，但是删掉一个 `p` 标签就可以了（chrome 版本 124.0.6367.208）：
...
当属性修改时，只有当该属性被访问时才会重新触发 `render`：
...
export default defineComponent({
  setup() {
    ...
    return () => {
      ...
      return (
        // 点击按钮不会重新触发 render
        <button onClick={() => (msg.value = "this is xyz.")}>click</button>
      );
    };
  },
});
...
export default defineComponent({
  setup() {
    ...
    return () => {
      ...
      return (
        // 点击按钮会重新触发 render
        <button onClick={() => (msg.value = "this is xyz.")}>
          {msg.value}
        </button>
      );
    };
...
当访问属性的元素被子组件包裹时，属性修改时不会重新触发 `render`，但是子组件会重新触发 `render`：
...
export default defineComponent({
  setup() {
    ...
    return () => {
      console.log("render...");
      return (
        // 点击按钮不会重新触发 render，但 Wrapper 组件会重新触发 render
        <Wrapper>
          <button onClick={() => (msg.value = "this is xyz.")}>
            {msg.value}
          </button>
